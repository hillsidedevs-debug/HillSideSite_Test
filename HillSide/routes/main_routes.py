...
def contact():
    form = ContactForm()
    if form.validate_on_submit():  # This now also validates reCAPTCHA!
        msg = Message(
            subject=f"New Contact Message from {form.name.data}",
            recipients=['hillside.devs@gmail.com'],
            body=f"""
...
{form.message.data}
        ...
        try:
            ...
            #app.logger.error(e)  # Better than print() in production
            flash('❌ Something went wrong while sending your message. Please try again later.', 'danger')
        
        return redirect(url_for('main.contact'))  # Redirect on success/failure
